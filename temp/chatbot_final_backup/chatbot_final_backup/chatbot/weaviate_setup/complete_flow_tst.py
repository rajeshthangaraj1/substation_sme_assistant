...
def sql_generation(weaviate_result, natural_language_query):
    ...
    for obj in weaviate_result:
        ...
        cleaned_resposne = clean_sql(response['response'].strip())
        # Extract SQL from response
        return cleaned_resposne, kpi_name
...
def format_results_to_natural_language(original_question: str, sql_results: tuple, kpi_name: str) -> str:
    ...
    prompt = f"""
        ...
        ### Example Output:
        1. The maximum utilization of VoWiFi Calls for yesterday was 4%.
        2. The node with maximum utilization for yesterday was :NODE with 1.84%.
        ...
        2. Use the Example Output just as samples to understand how can the result be formatted.
...
user_question = "What is the max utilization for HLR CP PROCESSOR LOAD for yesterday ?"
...
try:
    ...
    result_sql, kpi = sql_generation(result_query, user_question)
    print(f"result_sql is : {result_sql}")

    result_data = sql_query_executer(result_sql)
    print(f"result_data is : {result_data}")

    human_text = format_results_to_natural_language(user_question, result_data, kpi)
