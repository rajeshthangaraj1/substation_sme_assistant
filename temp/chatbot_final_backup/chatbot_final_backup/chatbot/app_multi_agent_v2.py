...
from logger_config_v1 import logger
...
if uploaded_file:
    ...
    print('file uploaded')
    with tempfile.NamedTemporaryFile(delete=False, suffix=f".{uploaded_file.name.split('.')[-1]}", dir = STATIC_TEMP_DIR) as tmp_file:
    ...
    if os.access(temp_file_path, os.R_OK):
        print(f"File {temp_file_path} is readable.")
    else:
        print(f"File {temp_file_path} is NOT readable.")

    if os.access(temp_file_path, os.W_OK):
        print(f"File {temp_file_path} is writable.")
    else:
        print(f"File {temp_file_path} is NOT writable.")

    if os.access(temp_file_path, os.X_OK):
        print(f"File {temp_file_path} is executable.")
    else:
        print(f"File {temp_file_path} is NOT executable.")

    response = file_handler_method.handle_file_upload(temp_file_path, original_filename)
    print(f'collection name {response.get("collection")}')
...
if user_input:
    ...
    context = {
        ...
        "is_sql_query": any(keyword in user_input.lower() for keyword in
                            ["utilization", "capacity", "vowifi", "report", "kpi", "node", "network"])
    }
